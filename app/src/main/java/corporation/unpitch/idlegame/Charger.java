...
public class Charger {
    ...
    //Si on veut charger des projets sauvegardés
    public static Projet chargerProjet(Context ctx, String nom_fichier){//La methode chargerObjet va retourner un objet de type Donnees
        try
        {
            FileInputStream fis = ctx.openFileInput(nom_fichier);
            ObjectInputStream ois = new ObjectInputStream(fis);
            Projet projet = (Projet) ois.readObject(); //le (Donnees) permet au ois.readObject de savoir que l'objet lu sera du type Donnees
            return projet;
        }
        catch(Exception ex)
        {
